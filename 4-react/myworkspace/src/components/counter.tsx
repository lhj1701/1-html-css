...
const Counter = () => {
  ...
  //const [state명, state 변경함수명 = useState(초기값)]
  // prop : 화면에 렌더링 결과에 영향을 미치는 변수, 외부에서 매개변수로 옴
  // state : 화면에 렌더링결과에 영향을 미치는 변수, 내부에서 선언함

  // state변수는 변경불가(immutable, 불변성)
  // count = 1; // 직접적인 변경불가
  // setCount(count+1); // state 변경함수로만 변경 가능

// state 변경 함수로 state를 변경했을 때만 컴포넌트를 업데이트함(다시 그림)
...
setCount(count + 1);
  ...
  return (
    <div>
      <h2>Counter</h2>
      <div>
        {/* button에 addEventListener("click", ()=>{})와 같음 */}
        <button onClick={()=>{
          increase();
        }}>COUNT</button>
        {/*  */}
