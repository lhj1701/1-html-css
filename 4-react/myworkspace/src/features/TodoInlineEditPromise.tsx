...
const Todo = () => {
  ...
  //useEffect : 특정 조건일 때 작동하는 코드를 발생할 수 있게하는 react hook
  ...
  // useEffect(이펙트를 처리할 함수, [의존변수])
  // 의존변수의 값/참조가 바뀔 때 마다 함수가 처리됨
  // ex)props가 바뀌거나 state가 바뀔 때 추가적인 처리를 함

  // []의존 변수 목록이 빈 배열 - 컴포넌트 렌더링되고 마운팅(렌더링)되는 시점에 처리가 됨
  useEffect(()=>{
// 의존변수가 바뀔 때 처리되는 코드를 작성
// [] -> 컴포넌트 로딩후에 바로 처리되는 코드
console.log("--1.mouted--");

// 백엔드에서 데이터를 받아올 것임
// ES6 style로 Promise 기법을 이용해서 데이터를 조회해옴
fetch("http://localhost:8080/todos")
//fetch 함수를 실행하고 네트워크 통신이 완료되면 then에 있는 함수(콜백-매개변수로 함수를 넘기면 콜백, 타입을 넘기면 쥐네릭)를 실행함
.then((res) => res.json()) // body가 json이면 js obect(array)로 변환
//응답데이터를 js object로 변환이 완료되면 다음 then에 있는 함수(콜백)을 실행함
//then에 있는 콜백함수의 매개변수로 변환된 결과를 넘겨줌
//서버에서 데이터를 받아오는 구조에 대한 interface를 별도로 정의하는 것이 좋음
.then((data : TodoItemResponse[]) => { // type aeertions 타입 단언
  // 백엔드에서 오는 데이터 구조와 state의 데이터 구조가 동일하다면 state객체로 바로 업데이트하면됨
  ...
  setTimeout(() => {
    ...
    setFetched(true); // 데이터받아옴 여부 state 업데이트
    setTodoList(todos);//todo state 업데이트
  }, 1000);
});
console.log("--3.complet--");
  },[]);
