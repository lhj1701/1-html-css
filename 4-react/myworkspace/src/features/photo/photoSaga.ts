import photoReducer, { addPhoto, initialCompleted, initialPagedPhoto, initialPhoto, modifyPhoto, PhotoPage, removePhoto } from "./photoSlice";
import { createAction, nanoid, PayloadAction } from "@reduxjs/toolkit";
import { PhotoItem } from "./photoSlice";
import { call, put, select, takeEvery, takeLatest } from "@redux-saga/core/effects";
import api, { PhotoItemRequest, PhotoItemResponse, PhotoPagingResponse } from "./photoApi";
...
function* addData(action: PayloadAction<PhotoItem>) {
  ...
  // redux state 조회하기
  const photoData : PhotoItem[] = yield select((state:Rootstate)=>state.photo.data);
  // 현재 데이터가 있으면
  ...
  // 얼럿박스를 추가해줌
  yield put(addAlert({id:nanoid(),variant:"success",message : "저장되었습니다."}))
...
localStorage.setItem("photo_page_size",size.toString())
  ...
  try{
     // 백엔드에서 데이터 받아오기
  const result: AxiosResponse<PhotoPagingResponse> = yield call(api.fetchPaging, page, size);
