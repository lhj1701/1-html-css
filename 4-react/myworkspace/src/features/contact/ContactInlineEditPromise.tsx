...
const ContactInlineEdit = () => {
  ...
  useEffect(()=>{
    // 의존 변수가 바뀔 때 처리되는 코드를 작성
    // [] -> 컴포넌트 로딩 후에 바로 처리되는 코드
    console.log("--1.mounted");
    // 백엔드에서 데이터를 받아올거임
    // ES 6 로 Promise 기법을 이용해서 데이터를 조회해옴
    fetch("http://localhost:8080/contacts")
    // fetch함수를 실행하고 네트워크 통신이 완료되면 then에 있는 함수(callback)를 실행함
    ...
    .then((data: ContactItemResponse[]) => {
      ...
      setTimeout(()=>{
        setLoading(false); // 로딩중이 state업데이트
        setContactList(contacts); //contacts state업데이트
      },300)
